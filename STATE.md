...
EPIC 6 — storage SQLite WAL (migration des JSONL provisoires `system_events.jsonl`/
`capture_events.jsonl`/l'état attribution vers de vraies tables), en autonomie via le
pattern build → audit → fix agents ([[vitrail-workflow]] côté mémoire). EPIC 7 reste un
squelette tant que CA/PolarProxy/keylog n'ont pas de vraie implémentation — chaque EPIC
suivant remplace son `StubSubsystem` par une implémentation réelle du trait `Subsystem`
sans toucher à l'orchestration (déjà validé trois fois : EPIC 7 → EPIC 2 → EPIC 1).
