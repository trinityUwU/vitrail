...
EPIC 3 — pipeline SSLKEYLOGFILE (décryptage TLS coopératif : injection de la variable
d'environnement, tail du fichier de clés, corrélation via `tshark`/parsing manuel), en
autonomie via le pattern build → audit → fix agents ([[vitrail-workflow]] côté mémoire).
Première source de contenu déchiffré réelle — `visibility.rs` (EPIC 5) est déjà prêt à
recevoir ce signal et à produire `Fully` sans modification. EPIC 7 reste un squelette pour
CA/PolarProxy tant qu'EPIC 4 n'est pas fait (dernier de l'ordre, le plus risqué).

**Point à surveiller** : le Mutex storage est désormais partagé entre `killswitch/`,
`capture/`, `attribution/` et maintenant `correlation/` (persistance de chaque `Flow`) —
couplage de robustesse identifié en EPIC 6, toujours pas bloquant, mais la charge d'écriture
augmente à chaque EPIC. Si EPIC 3/4 ajoutent encore du volume, un vrai pool de connexions
