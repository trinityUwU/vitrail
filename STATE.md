...
EPIC 1 — attribution processus (serveur gRPC `ui.proto`, crate `tonic` — attention, le
daemon `opensnitchd` est le CLIENT gRPC, `attribution/` doit implémenter le SERVEUR), en
autonomie via le pattern build → audit → fix agents ([[vitrail-workflow]] côté mémoire).
EPIC 7 reste un squelette tant que CA/PolarProxy/attribution/keylog n'ont pas de vraie
implémentation — chaque EPIC suivant remplace son `StubSubsystem` par une implémentation
réelle du trait `Subsystem` sans toucher à l'orchestration (déjà validé deux fois : EPIC 7
→ EPIC 2).
