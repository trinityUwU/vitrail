...
EPIC 5 — corrélation timeline (fusion capture + attribution + storage en un flux `Flow`
exploitable, première alimentation réelle des tables `flows`/`processes` restées vides
depuis EPIC 6), en autonomie via le pattern build → audit → fix agents
([[vitrail-workflow]] côté mémoire). EPIC 7 reste un squelette tant que CA/PolarProxy/
keylog n'ont pas de vraie implémentation.

**Point à surveiller** : le Mutex storage est désormais partagé entre `killswitch/`,
`capture/` et `attribution/` — l'audit EPIC 6 a signalé un couplage de robustesse nouveau
(le filet de sécurité `AbnormalExitGuard` d'attribution dépend maintenant indirectement de
l'état du Mutex storage partagé avec capture) sans le juger bloquant. À garder en tête si
EPIC 5/3/4 ajoutent encore plus de charge d'écriture sur cette même connexion unique — un
passage à un vrai pool de connexions pourrait devenir nécessaire.
